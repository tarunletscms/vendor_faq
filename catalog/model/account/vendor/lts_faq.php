...
class ModelAccountVendorLtsFaq extends Model {
    ...
    public function getFaqs($data) {
        $data_faqs=array();
        $sql="SELECT * FROM " . DB_PREFIX . "lts_faq f LEFT JOIN ".DB_PREFIX."lts_faq_description fd on f.faq_id=fd.faq_id where fd.language_id='".(int)$this->config->get('config_language_id')."' LIMIT ". $data['start'] .' , '. $data['limit'];
