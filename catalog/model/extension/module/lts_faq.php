...
class ModelExtensionModuleLtsFaq extends Model {
	public function addFaq($data) {
	    ...
	    $this->db->query("INSERT INTO " . DB_PREFIX . "lts_faq SET image = '" . $this->db->escape($data['image']) . "', `status` = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
    ...
    public function getFaqs($limit,$product_id) {
        $data_faqs=array();
        $sql="SELECT * FROM " . DB_PREFIX . "lts_faq f LEFT JOIN ".DB_PREFIX."lts_faq_description fd on f.faq_id=fd.faq_id where fd.language_id='".(int)$this->config->get('config_language_id')."' and f.product_id='".$product_id."' LIMIT $limit";
